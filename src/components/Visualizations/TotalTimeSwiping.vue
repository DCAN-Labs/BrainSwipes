<template>
  <div>
    ...
    <div v-else id="totalTimeSwiping">
      <div id="datePicker">
    ...
    methods: {
      async createParentChart(dataset) {
        ...
        const reducedVotes = _.reduce(votes, (result, value) => {
          if (Object.hasOwn(value, 'datetime')) {
            ...
            if (Object.hasOwn(dateRangeObject, day)) {
              (result[day] || (result[day] = [])).push({ user: value.user, time: value.time });
            }
          }
          return result;
        }, dateRangeObject);
        ...
        const votesPerDay = _.reduce(reducedVotes, (result, value, key) => {
          const timeVoting = _.reduce(value, (timeResult, timeValue) => {
            timeResult += timeValue.time;
            return timeResult;
          }, 0);
          result[key] = Math.round(timeVoting / 1000);
          return result;
        }, {});
        ...
        const options = {
          ...
          colors: colorsArray,
          title: {
            text: 'Time Spent Swiping per Day (seconds)',
          },
          subtitle: {
      ...
      createChildChart(dataPoint, color) {
        const timePerUser = _.reduce(this.votesByDay[dataPoint], (result, value) => {
          result[value.user] = result[value.user] ? result[value.user] + value.time : value.time;
          return result;
        }, {});
        const secondsPerUser = _.reduce(timePerUser, (result, value, key) => {
          result[key] = Math.round(value / 1000);
          return result;
        }, {});
        const dataKeys = Object.keys(secondsPerUser);
        const dataValues = Object.values(secondsPerUser);
        const options = {
          chart: {
            type: 'bar',
            height: 350,
          ...
          colors: color,
          title: {
            text: `Seconds Spent Swiping on ${dataPoint} by user`,
          },
          stroke: {
            show: true,
            width: 2,
