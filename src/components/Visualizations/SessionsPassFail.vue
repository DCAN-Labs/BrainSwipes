<template>
  <div>
    ...
    <div v-else id="sessionsPassFail">
      <div>
        <p>Download a CSV file listing Session, T1, T2, Rest, All. 1 for pass, 0 for fail.</p>
    ...
    methods: {
      ...
      generateCSV() {
        ...
        const csv = _.reduce(this.passedSessions, function(result, value, key){
          ...
          const Rest = value.hasOwnProperty('Rest') ? value['Rest'] ? 1 : 0 : '';
          const All = value.hasOwnProperty('All') ? value['All'] ? 1 : 0 : '';
          result += `${key},${T1},${T2},${Rest},${All}\n`;
          return result;
        },'Session,T1,T2,Rest,All\n');
        /* eslint-enable */
        console.log(csv);
