    ...
    methods: {
      ...
      addDefinitions() {
        ...
        Object.keys(this.glossary).forEach((category) => {
          terms = terms.concat(Object.keys(this.glossary[category]));
        });
        console.log(terms);
        Object.keys(this.steps).forEach((category) => {
          console.log(category);
          Object.keys(category).forEach((step) => {
            if (Object.hasOwnProperty.call(this.steps[category], step)) {
              // console.log(this.steps[category][step].text);
              terms.forEach((term) => {
                const match = this.steps[category][step].text.toString().toUpperCase().match(term.toString().toUpperCase());
                if (match) {
                  console.log(match);
                  const regex = new RegExp(term,"gi");
                  const result = this.steps[category][step].text.toString().replace(regex, `<dfn title="test">${term}</dfn>`);
                  console.log(result);
                  this.steps[category][step].text = result;
                }
              });
            }
          });
